...
function writeCoverage(aggregate) {
  ...
  const htmlReport = getHTMLReport(junitXml, { reportTitle: 'Patternfly axe report' });
  ...
  // Exit code
  if (suites.find(suite => suite.testCases.find(test => test.failures.length > 0))) {
    return 1;
  }
  if (suites.find(suite => suite.testCases.find(test => test.incomplete && test.incomplete.length > 0))) {
    return 2;
  }
