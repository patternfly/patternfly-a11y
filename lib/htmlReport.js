...
var generateTDTable = function (reportXml) {
    ...
    for (var j in testcases) {
      ...
      if (testFailed) {
        testDetailsTable +=
          ...
          entity(testFailed.val) +
          "</pre></td>";
      } else if (testSkipped) {
        testDetailsTable +=
          '<td id="td-table-test-skip">Skipped</td><td><pre>' +
          entity(testSkipped.val) +
          "</pre></td>";
      } else if (testError) {
        testDetailsTable +=
          '<td id="td-table-test-fail">Error</td><td><pre>' +
          entity(testError.val) +
          "</pre></td>";
      } else {
  ...
  testExecInfo["passRate"] = (
    (totalTests - totalFailures - totalSkips) /
    totalTests
  ).toFixed(3);
...
var generateTSTable = function (testConfig) {
  ...
  testSummaryTable +=
    "<li><b>Test Start:</b> " + testExecInfo["testStartedOn"] + "</li>";
  testSummaryTable +=
    "<li><b>Total Tests:</b> " + testExecInfo["totalTests"] + "</li>";
  testSummaryTable +=
    "<li><b>Pass Rate:</b> " + testExecInfo["passRate"] * 100 + "% </li>";
  testSummaryTable +=
    "<li><b>Execution Duration:</b> " + testExecInfo["execTime"] + " Secs</li>";
...
function getHTMLReport(reportXml, testConfig) {
  ...
  var htmlReport =
    "<html><head>" + reportTitle + reportCss + reportScript + "</head>";
